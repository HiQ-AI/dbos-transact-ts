...
describeNativeSQLite('SQLite system database', () => {
  ...
  test('upgrades a version 107 SQLite schema without losing legacy payloads or steps', async () => {
    const migrationDbUrl = sqliteUrlFor(path.join(tempDir, 'behind-migrations.sqlite'));
    const pool = new SQLitePool(migrationDbUrl);
    try {
      // Actual pre-split table shapes, not a current database with its version reset.
      await pool.query(`CREATE TABLE dbos_migrations (version INTEGER PRIMARY KEY)`);
      await pool.query(`INSERT INTO dbos_migrations VALUES (107)`);
      await pool.query(`CREATE TABLE queues (name TEXT PRIMARY KEY, polling_interval_sec REAL)`);
      await pool.query(`INSERT INTO queues VALUES ('behind-queue', 1)`);
      await pool.query(`CREATE TABLE workflow_status (
        workflow_uuid TEXT PRIMARY KEY, inputs TEXT, output TEXT, error TEXT)`);
      await pool.query(`INSERT INTO workflow_status VALUES ('legacy', '[1]', '2', NULL)`);
      await pool.query(`CREATE TABLE operation_outputs (
        workflow_uuid TEXT NOT NULL, function_id INTEGER NOT NULL,
        function_name TEXT NOT NULL DEFAULT '', output TEXT, error TEXT, child_workflow_id TEXT,
        started_at_epoch_ms INTEGER, completed_at_epoch_ms INTEGER,
      ...
      await pool.query(`INSERT INTO operation_outputs
        (workflow_uuid, function_id, function_name, output, application_name)
        VALUES ('legacy', 1, 'old-step', '2', 'old-app')`);
    } finally {
      await pool.end();
    }
    ...
    try {
      ...
      expect(await readSQLiteMigrationVersion(migrationDbUrl)).toBe(allMigrations().length);
      expect((await verifyPool.query('SELECT inputs, output FROM workflow_status')).rows).toEqual([
        { inputs: '[1]', output: '2' },
      ]);
      await verifyPool.query(`UPDATE queues SET partition_concurrency = 2,
        partition_worker_concurrency = 1, partition_rate_limit_max = 4, partition_rate_limit_period_sec = 1`);
      await verifyPool.query(`INSERT INTO workflow_input (workflow_uuid, inputs) VALUES ('new', '[3]')`);
      await verifyPool.query(`INSERT INTO workflow_output (workflow_uuid, output) VALUES ('new', '4')`);
      const step = await verifyPool.query<{ output: string; application_name: string; retention_timestamp: number }>(
        'SELECT output, application_name, retention_timestamp FROM operation_outputs',
      );
      expect(step.rows[0]).toMatchObject({ output: '2', application_name: 'old-app' });
      expect(step.rows[0].retention_timestamp).toBeGreaterThan(0);
      expect((await verifyPool.query('PRAGMA foreign_key_list(operation_outputs)')).rows).toEqual([]);
      await verifyPool.query(`DELETE FROM workflow_status WHERE workflow_uuid = 'legacy'`);
      expect((await verifyPool.query('SELECT * FROM operation_outputs')).rows).toHaveLength(1);
      expect((await verifyPool.query('PRAGMA index_list(operation_outputs)')).rows).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ name: 'idx_operation_outputs_retention' }),
          expect.objectContaining({ name: 'idx_operation_outputs_completed_at_function_name' }),
        ]),
      );
    } finally {
      await verifyPool.end();
  ...
  test('queue claim respects a per-poll dispatch budget on SQLite', async () => {
    ...
    await exec.dispatchDequeuedWorkflows([...firstClaim, ...secondClaim, ...finalClaim]);
