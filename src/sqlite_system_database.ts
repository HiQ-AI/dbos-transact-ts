...
/** Verify without creating a database, changing its journal mode, or applying migrations. */
  ...
  try {
    ...
    if (currentVersion < requiredVersion) {
      throw new DBOSInitializationError(
        `SQLite system database is at schema version ${currentVersion}, but this version of DBOS requires ${requiredVersion}. ` +
...
function sqliteMigrations(): ReadonlyArray<SQLiteMigration> {
  return [
    {
      // Fixed PR #1288 baseline; new upstream migrations need explicit SQLite equivalents.
      version: 107,
