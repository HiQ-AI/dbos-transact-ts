...
type NativeSQLiteModule = {
  DatabaseSync: new (path: string, options?: { readOnly?: boolean }) => NativeSQLiteDatabase;
};
...
export class SQLitePool {
  ...
  private database?: NativeSQLiteDatabase;
  ...
  private get db(): NativeSQLiteDatabase {
    if (this.ended) throw sqlitePoolEndedError();
    if (this.database) return this.database;
    const file = sqliteFileFromUrl(this.databaseUrl);
    const { DatabaseSync } = loadNativeSQLite();
    const database = new DatabaseSync(file === ':memory:' ? file : path.resolve(file));
    try {
...
/** Verify without creating the database, changing its journal mode, or applying migrations. */
export async function verifySQLiteSystemDatabase(
  sysDbUrl: string,
  logger: GlobalLogger,
  schemaName: string = 'dbos',
): Promise<void> {
  ...
  if (file === ':memory:') {
    throw new DBOSInitializationError('runMigrations disabled requires an existing file-backed SQLite system database');
  }
  const { DatabaseSync } = loadNativeSQLite();
  const db = new DatabaseSync(path.resolve(file), { readOnly: true });
  try {
    const currentVersion = await getCurrentSQLiteMigrationVersion(new SQLiteClient(db, schemaName));
    const requiredVersion = latestSQLiteMigrationVersion();
    if (currentVersion < requiredVersion) {
      throw new DBOSInitializationError(
        `SQLite system database is at schema version ${currentVersion}, but DBOS requires ${requiredVersion}. ` +
          'runMigrations disabled: migrate out of band (`npx dbos schema`) or enable runMigrations.',
      );
    }
    logger.debug(`SQLite system database schema version ${currentVersion} satisfies ${requiredVersion}`);
  } finally {
    db.close();
  }
}
...
function sqliteMigrations(): ReadonlyArray<SQLiteMigration> {
  ...
  return [
    {
      // PR #1288's schema snapshot. Never advance this number with PostgreSQL migrations.
      version: 107,
    ...
    {
      version: 109,
      // Retain legacy payload columns: latest readers coalesce them with the split tables.
      statements: [
        `CREATE TABLE workflow_input (
          workflow_uuid TEXT NOT NULL PRIMARY KEY, inputs TEXT,
          retention_timestamp INTEGER NOT NULL DEFAULT ${now})`,
        `CREATE TABLE workflow_output (
          workflow_uuid TEXT NOT NULL PRIMARY KEY, output TEXT, error TEXT,
          retention_timestamp INTEGER NOT NULL DEFAULT ${now})`,
        `CREATE INDEX idx_workflow_input_retention ON workflow_input (retention_timestamp)`,
        `CREATE INDEX idx_workflow_output_retention ON workflow_output (retention_timestamp)`,
      ],
    },
    {
      version: 110,
      // SQLite cannot ADD COLUMN with a nonconstant default on populated tables.
      // Rebuild also removes the workflow_status FK (PostgreSQL migration 112).
      statements: [
        `CREATE TABLE operation_outputs_retention (
          workflow_uuid TEXT NOT NULL, function_id INTEGER NOT NULL,
          function_name TEXT NOT NULL DEFAULT '', output TEXT, error TEXT,
          child_workflow_id TEXT, started_at_epoch_ms INTEGER, completed_at_epoch_ms INTEGER,
          serialization TEXT DEFAULT NULL, application_name TEXT DEFAULT NULL,
          retention_timestamp INTEGER NOT NULL DEFAULT ${now},
          PRIMARY KEY (workflow_uuid, function_id))`,
        `INSERT INTO operation_outputs_retention (
         ...
         SELECT workflow_uuid, function_id, function_name, output, error, child_workflow_id,
          started_at_epoch_ms, completed_at_epoch_ms, serialization, application_name,
          ${now} FROM operation_outputs`,
        `DROP TABLE operation_outputs`,
        `ALTER TABLE operation_outputs_retention RENAME TO operation_outputs`,
    ...
    {
      version: 111,
      statements: [`CREATE INDEX idx_operation_outputs_retention ON operation_outputs (retention_timestamp)`],
    },
    { version: 112, statements: [] }, // FK removed by the atomic SQLite table rebuild in 110.
    { version: 113, statements: [] }, // PostgreSQL enqueue stored function; SQLite uses SDK writes.
  ];
}

function latestSQLiteMigrationVersion(): number {
  return sqliteMigrations().at(-1)!.version;
}
  ...
  try {
    ...
    for (const migration of migrations) {
      ...
      try {
        ...
        if (migration.version === 107) await ensureSQLiteApplicationNameSchema(client);
