...
describeNativeSQLite('SQLite system database', () => {
  ...
  test('upgrades a version 107 queue schema while preserving its rows', async () => {
    const migrationDbUrl = sqliteUrlFor(path.join(tempDir, 'behind-migrations.sqlite'));
    const pool = new SQLitePool(migrationDbUrl);
    try {
      // A genuine pre-108 shape: no partition limit columns. Resetting a current
      // database's version would not exercise ALTER TABLE or preservation of old data.
    ...
    try {
      ...
      const before = await verifyPool.query(
        'SELECT partition_concurrency, partition_worker_concurrency, partition_rate_limit_max, partition_rate_limit_period_sec FROM queues',
      );
      ...
      await verifyPool.query(`UPDATE queues SET partition_concurrency = 2,
        partition_worker_concurrency = 1, partition_rate_limit_max = 3, partition_rate_limit_period_sec = 0.5`);
      expect((await verifyPool.query('SELECT partition_rate_limit_period_sec FROM queues')).rows).toEqual([
        { partition_rate_limit_period_sec: 0.5 },
      ]);
    } finally {
      await verifyPool.end();
    }
